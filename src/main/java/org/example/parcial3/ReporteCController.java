...
public class ReporteCController {
    ...
    public String reporte() throws SQLException {//00377223 regresa el reporte en cadena de texto, para las demas funciones
        pst = cn.prepareStatement("SELECT t.numero_tarjeta, tt.nombre\n" +//00377223 se seleccionan los datos que seran parte del reporte
                "FROM \n" +//00377223 se seleccionara de:
                "TARJETA t \n" +//00377223 se declara un alias para la tabla TARJETA
                "INNER JOIN TIPO_TARJETA tt ON t.FK_id_tipo_tarjeta = tt.id\n" +//00377223 se hace un inner join para tener el conjunto donde el id de TIPO_TARJETA y la fk del id tipo_tarjeta en TARJETA coincidan y poder agarrar los datos del SELECT
                "WHERE t.FK_id_cliente = ?;");//00377223 con la condicion del id cliente para el reporte
        ...
        while (rs.next()) {//00377223 se recorreran todas la filas del resultado de la query
            ...
            String tipoTarjeta = rs.getString("nombre");//00377223 se guarda el tipo de tarjeta de la iteracion
            ...
            if (tipoTarjeta.equals("Crédito")){//00377223 condicion para tarjetas tipo Credito
                credito.add(tarjetaCensurada);//00377223 se agrega a la lista que se creo antes
            }
            else if (tipoTarjeta.equals("Débito")){//00377223 condicion para tarjetas tipo Debito
